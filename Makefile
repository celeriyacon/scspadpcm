...
M68K_CFLAGS=-Os -g -std=gnu99 -march=68000 -fwrapv -fsigned-char -fno-strict-aliasing -Wall -Werror -Werror="stack-usage=" -Wstack-usage=64
...
adp68k-data.h:	Makefile gen-data.cpp dsp-macros.h tables.h
		$(CXX) $(CXXFLAGS) $(CPPFLAGS) -o gen-data gen-data.cpp
		./gen-data > adp68k-data.h

adpencode:	Makefile adpencode.cpp types.h tables.h
...
test.bin:	Makefile samples.ladp.inc test.c test.ld adp68k.h adp68k.bin.inc scsp.h dsp-macros.h types.h sh2.h smpc.h
		$(SH2_CC) $(SH2_CFLAGS) -nostdlib -Xlinker -Ttest.ld -o test.elf test.c -lgcc && $(SH2_OBJCOPY) -O binary test.elf test.bin
