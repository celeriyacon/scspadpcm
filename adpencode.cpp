...
static inline int32 EncodeBlock(ADContext* c, const int16* input, int16* output, uint8* block_data)
{
...
int main(int argc, char* argv[])
{
 ...
 for(int32 block_counter = 0; block_counter < (sfi.frames + 15) >> 4; block_counter++)
 {
  ...
  total_error += EncodeBlock(&c, inbuf, outbuf, block_data);
  ...
  if(outsf)
   sf_write_short(outsf, outbuf, c.samples_per_block);
 }
